$(document).ready(() => {
  ...
  $('.btn-delete').click(function() {
    ...
    let r = false;
    if (type === 'venue') {
      const deleteVenue = venues.find(venue => venue._id === id);
      ...
      const deleteUser = users.find(user => user._id === id);
    ...
    if (r === true) {
      $.ajax({
        ...
        success: (data) => {
          console.log(data);
          if (type === 'venue') {
            venues = venues.filter(venue => venue._id !== id);
      ...
      success: (data) => {
        ...
        if (reqType === 'POST') {
          ...
          if (type === 'venue') {
            ...
            const findVenue = venues.findIndex(venue => venue._id === data._id);
            venues[findVenue] = data;
          } else if (type === 'user') {
            const findUser = users.findIndex(user => user._id === data._id);
            users[findUser] = data;
          }
        }
        $(this).closest('.modal').modal('toggle');
...
$(function () {
  $(document).on('click', '.edit-row', (function() {
    ...
    if (type === 'venue') {
      const obj = venues.find(venue => venue._id === objId);
      console.log(obj);
      $('#venueId').val(obj._id);
      ...
      const obj = users.find(user => user._id === objId);
      console.log(obj);
      $('#email').val(obj.email);
      $('#userId').val(obj._id);
